...
Initialise::Initialise(int windowWidth, int windowHeight, int playerLives, int numEnemies, int numBases)
{
	...
	//1: GAME PARAMETERS
		...
		CreatePlayer(playerLives, windowWidth, windowHeight);
...
void Initialise::CreatePlayer(int lives, int width, int height) {
	//	1.3.1.1: Set the pointer of the player to its instantiated object
	playerObjectPtr = new Player(lives, true, 0, playerObjectPtr->Laser);
	...
	//	1.3.1.6: Only initialise the AABB of the sprite 
...
void Initialise::CreateEnemy(int number, int width, int height) {
	for (int i = 0; i < number; i++) {
		...
		enemyPtr->SetPosition(
			(width / (numberOfEnemies + 1)) * (i + 1) - (enemySpritePtr->Width() / 2 ),
			height * 0.05f);
